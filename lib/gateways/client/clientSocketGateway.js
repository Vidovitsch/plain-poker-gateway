...
/**
  ...
  */
C.sendLobbyReply = function sendLobbyReply(client, replyData, request) {
  const context = 'lobby-reply';
  const message = new RpcMessage(context, replyData).toReply(request);
  client.emit(context, JSON.stringify(message));
};
...
C.onLobbyRequest = function onLobbyRequest(client, callback) {
  client.on('lobby-request', (request) => {
    callback(request);
  });
};
