 ...
 */
TableGateway.prototype.requestTableCreation = async function(data, sessionId) {
    return new Promise((resolve, reject) => {
        // Ensure the data has been sent before listening to a reply
        this.sender.send('create-table-request', JSON.stringify({
            data: data,
            sessionId: sessionId,
        })).then(() => {
            this.receiver.startReceiving(util.format('client_%s_%s', data.name, sessionId), sessionId, (err, data, conn) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(data);
