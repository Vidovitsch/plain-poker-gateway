...
A.listen = function(channel, queue, callback) {
    ...
    this.assertQueueAsync(channel, queue, {autoDelete: true}).then((q) => {
        ...
        channel.consume(q.queue, (message) => {
            if (!message.content.properties.status !== 'reply') {
                        console.log('here');
 ...
 * @return {Promise}         [description]
 */
A.rpcAsync = function(message, sendTo) {
    return new Promise((resolve, replyTo, reject) => {
        // Create promises
        const connectionPromise = this.connectAsync();
        const channelPromise = connectionPromise.then((connection) => {
            return this.createChannelAsync(connection);
        ...
        const queuePromise = channelPromise.then((channel) => {
            return this.assertQueueAsync(channel, replyTo, {exclusive: true});
        });
        ...
        Promise.all([connectionPromise, channelPromise, queuePromise]).then(([connection, channel, q]) => {
            ...
            channel.consume(q.queue, (message) => {
                if (message.content.properties.correlationId === correlationId && message.content.propteries.status === 'reply') {
                    channel.ack(message);
                    setTimeout(() => {
                        connection.close();
                    }, 500);
            ...
            message.properties.isRpc = true;
            message.properties.replyTo = replyTo;
            message.properties.status = 'request';
