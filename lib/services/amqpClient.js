 ...
 */
A.listen = function listen(args, callback) {
  args.channel.assertExchange(this.exchange, 'direct', { durable: true });
  this.assertQueueAsync(args.channel, args.queue, { autoDelete: true }).then((q) => {
    if (args.routingKey) {
      this.bindQueue(args.channel, q.queue, args.routingKey);
    } else {
      this.bindQueue(args.channel, q.queue, args.context);
    }
    args.channel.consume(q.queue, (message) => {
      args.channel.ack(message);
    ...
    debug(`Continuous listener started => [queue:${args.queue}]`);
  }).catch((err) => {
    callback(err);
  });
};
...
A.rpcAsync = function rpcAsync(requestMessage, sendTo) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all([connectionPromise, channelPromise, queuePromise]).then(([connection, channel, q]) => {
      ...
      debug(`(request) Message sent => [destination:${sendTo}], [reply:${replyTo}], [context:${requestMessage.context}], [correlation:${requestMessage.correlationId}]`);
    }).catch((err) => {
      reject(err);
    });
  });
...
A.closeSharedConnection = function closeSharedConnection(key) {
  ...
  if (sharedConnection) {
    ...
    delete this.sharedConnections[key];
    debug(`(shared) Connection closed => [key:${key}]`);
  }
};

A.closeSharedChannel = function closeSharedChannel(key) {
  const sharedChannel = this.sharedChannels[key];
  if (sharedChannel) {
    sharedChannel.close();
    delete this.sharedChannels[key];
    debug(`(shared) Channel closed => [key:${key}]`);
  }
};
